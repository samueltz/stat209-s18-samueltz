...
**Answer**:The distribution is concentrated between 10 and 17. Even though the variable is nmueric, it is limited to whole number between 1 and 20.
...
model <- lm_basic(warmth ~ 1, data = HU)
reg_table(model, level = 0.95)
```
...
**Answer**:This confidence interval is broader than the previous one. This could be because it looks at a smaller set of data. 
...
```{r}
model <- lm_basic(warmth ~ 1 + gender, data = HU)
reg_table(model, level = 0.95)
```
...
```{r}
model <- lm_basic(warmth ~ 1 + fourties, data = age)
reg_table(model, level =0.95)
```
...
**Answer**:This shows that people in their fourties are generally more warm than people in their thirties. I don't know of any stereotypes refering to warmth and age, but this could give ground to a stereotype.
...
**Answer**:The AU has the highest warmth score.
...
**Answer**:The slope is statistically lower than one. This means that sensitivity has a lower than expected effect on the baseline score.

Fit a regression model that uses both gender and the baseline score to
predict your trait.

```{r}
model <- lm_basic(warmth ~ 1 + gender + baseline, data = pf)
reg_table(model, level = 0.95)
```

Describe the slope for the gender term in words:

**Answer**: The warmth inceases by about 0.9984 for each one point of warmth.

Add predictions from the previous model to `pf`.

```{r}
predictions <- add_prediction(pf, model)
```
...
```{r}
ggplot(predictions, aes(model_pred, baseline)) +
  geom_point(aes(color = gender))
```

What do the predicted values look like?

**Answer**:The graph is a stright linear line that is increasing.
