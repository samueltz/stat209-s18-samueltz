...
There are 154 observations in the data. Each row is the summarized data for each area at both day and nightime. The slopes are different. Daytime burglaries occur evenly accross all incomes, while nighttime burglaries increases in rate as income lowers.
...
```{r}
crimes_2 <- group_summarize(crimes, area_number, night_flag)
crimes_2 <- left_join(crimes_2, ca, by = "area_number")
crimes_2$assault_rate <- crimes_2$assault_sum / crimes_2$num_households * 1000

ggplot(crimes_2, aes(median_income, assault_rate)) +
  geom_point(aes(color = factor(night_flag))) +
  geom_smooth(method = "lm", aes(color = factor(night_flag))) +
  geom_label(label = "Loop", x = 65000, y = 21)

```
Across all area numbers, assault rates occur at a constant rate for all incomes, however during the night time, assault rates spike for areas of lower income. One exception to this trend is the Loop area, which experiences unusually high assault rate during night time for its median income level. 
