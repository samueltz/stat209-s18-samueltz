...
**Answer**:3rd: 369 sec, 97th: 4107.46
...
**Answer**:both slopes are significantly higher than the fast group.
...
```{r}
pf$new <- fct_relevel(pf$speed, "slow")
model <- lm_basic(perfectionism ~ 1 + new, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**:There is no significant value between slow and normal speeds.
...
**Answer**: The people who finish quick may not read the questions, and therefore not actually hav valid results, thus giving low confidence. People who take a long time probably let the test sit for a while, maybe because it was started and finished later.
...
```{r}
model <- lm_basic(warmth ~ 1 + baseline + social_boldness + dominance, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**:The social boldness trait mathches up with what I thought, and increases as warmth increases, dominance was not significant.

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(warmth ~ 1 + baseline + social_boldness + dominance + speed + gender + fct_lump(country, n=5), data = pf)
reg_table(model, level = 0.95)
```

Does this change the slopes for your two traits much?

**Answer**:Yes, it makes them both significant

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**:The table shows the relationship between the traits, but takes itno account a variety of other factors that werent considered before.
